...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			if ($this->user->hasPermission('access', 'report/customer_blog_search')) {
				$report_customer[] = array(
					'name'     => $this->language->get('text_report_customer_blog_search'),
					'href'     => $this->url->link('report/customer_blog_search', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
